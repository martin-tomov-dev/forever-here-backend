...
exports.sendSMS = async (phoneNumber, message) => {
  ...
  sns.publish(params, (err, data) => {
    if (err) {
      ...
      checkMessageStatus(data.MessageId);
    }
  });
};
...
const checkMessageStatus = async (messageId) => {
  ...
  try {
    const response = await sns1.listPhoneNumbersOptedOut().promise();
    const optedOutPhoneNumbers = response.phoneNumbers;

    if (optedOutPhoneNumbers.includes(messageId)) {
      console.log("Message status: Opted Out");
    } else {
      console.log("Message status: Sent or Delivered");
    }
  } catch (error) {
